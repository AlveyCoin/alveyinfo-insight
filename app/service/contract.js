...
class ContractService extends Service {
  ...
  async getContractSummary(contractAddress, addressIds) {
    const {Address, Contract, Qrc20: ARC20, Qrc20Balance: ARC20Balance, Qrc721: ARC721} = this.ctx.model
    ...
    if (contract.type === 'arc20') {
      contract.arc20.holders = await ARC20Balance.count({
        where: {
          contractAddress,
    ...
    let [
      ...
      arc20Service.getAllARC20Balances([contractAddress]),
      arc721Service.getAllARC721Balances([contractAddress]),
    ...
    return {
      ...
      ...contract.type === 'arc20' ? {
        arc20: {
          name: contract.arc20.name,
          symbol: contract.arc20.symbol,
          decimals: contract.arc20.decimals,
          totalSupply: contract.arc20.totalSupply,
          version: contract.arc20.version,
          holders: contract.arc20.holders
        }
      } : {},
      ...contract.type === 'arc721' ? {
        arc721: {
          name: contract.arc721.name,
          symbol: contract.arc721.symbol,
          totalSupply: contract.arc721.totalSupply
        }
      } : {},
    ...
    let result = await db.query(sql`
      SELECT COUNT(*) AS count FROM (
        ...
        WHERE receipt_log.receipt_id = receipt._id
          AND contract.address = receipt_log.address AND contract.type IN ('arc20', 'arc721')
          AND receipt_log.topic1 = ${TransferABI.id}
          AND (receipt_log.topic2 = ${topic} OR receipt_log.topic3 = ${topic})
          AND (
            (contract.type = 'arc20' AND receipt_log.topic3 IS NOT NULL AND receipt_log.topic4 IS NULL)
            OR (contract.type = 'arc721' AND receipt_log.topic4 IS NOT NULL)
          )
      ) list
