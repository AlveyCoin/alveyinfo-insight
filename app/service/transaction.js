...
class TransactionService extends Service {
          ...
          outputObject.receipt = {
            ...
            logs: eventLogs.filter(log => log.receiptId === output.receipt._id).map(log => ({
              ...
              ...log.arc20 ? {
                arc20: {
                  name: log.arc20.name,
                  symbol: log.arc20.symbol,
                  decimals: log.arc20.decimals
                }
              } : {},
              ...log.arc721 ? {
                arc721: {
                  name: log.arc721.name,
                  symbol: log.arc721.symbol
                }
              } : {}
            }))
          }
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, Script} = this.app.alveyinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.alveyinfo.rpc(this.app.config.alveyinfo.rpc)
  ...
  async transformTransaction(transaction, {brief = false} = {}) {
    ...
    let [arc20TokenTransfers, arc721TokenTransfers] = await Promise.all([
      this.transformARC20Transfers(transaction.outputs),
      this.transformARC721Transfers(transaction.outputs)
    ])
    ...
    return {
      ...
      ...brief ? {} : {
        ...
        scriptSig: {
          ...
          asm: this.app.alveyinfo.lib.Script.fromBuffer(
            input.scriptSig,
  ...
  transformOutput(output, index, {brief}) {
    const {Script} = this.app.alveyinfo.lib
  ...
  async transformARC20Transfers(outputs) {
    const TransferABI = this.app.alveyinfo.lib.Solidity.arc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.receipt) {
        for (let {address, addressHex, topics, data, arc20} of output.receipt.logs) {
          if (arc20 && topics.length === 3 && Buffer.compare(topics[0], TransferABI.id) === 0 && data.length === 32) {
  ...
  async transformARC721Transfers(outputs) {
    const TransferABI = this.app.alveyinfo.lib.Solidity.arc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.receipt) {
        for (let {address, addressHex, topics, arc721} of output.receipt.logs) {
          if (arc721 && topics.length === 4 && Buffer.compare(topics[0], TransferABI.id) === 0) {
            let [from, to] = await this.ctx.service.contract.transformHexAddresses([topics[1].slice(12), topics[2].slice(12)])
            result.push({
              address,
              addressHex: addressHex.toString('hex'),
              name: arc721.name,
              symbol: arc721.symbol,
              ...from && typeof from === 'object' ? {from: from.string, fromHex: from.hex.toString('hex')} : {from},
              ...to && typeof to === 'object' ? {to: to.string, toHex: to.hex.toString('hex')} : {to},
