...
class ARC20Service extends Service {
  async listARC20Tokens() {
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    let {limit, offset} = this.ctx.state.pagination

    let result = await db.query(sql`
      SELECT COUNT(DISTINCT(arc20_balance.contract_address)) AS count FROM arc20_balance
      INNER JOIN arc20 USING (contract_address)
    ...
    let list = await db.query(sql`
      SELECT
        ...
        arc20.name AS name, arc20.symbol AS symbol, arc20.decimals AS decimals, arc20.total_supply AS totalSupply,
        arc20.version AS version,
  ...
  async getAllARC20Balances(hexAddresses) {
    if (hexAddresses.length === 0) {
      return []
    }
    const {Contract, Qrc20: ARC20, Qrc20Balance: ARC20Balance} = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    let list = await ARC20.findAll({
    ...
    return list.map(item => ({
      ...
      symbol: item.symbol,
      decimals: item.decimals,
      balance: item.contract.arc20Balances.map(({balance}) => balance).reduce((x, y) => x + y)
    })).filter(({balance}) => balance)
  }

  async getARC20BalanceHistory(addresses, tokens) {
    ...
    const {Header, Transaction, Receipt, ReceiptLog, Contract, Qrc20: ARC20, Qrc20Balance: ARC20Balance, literal} = db
    ...
    let result = await db.query(sqlRaw`
      ...
      FROM receipt, receipt_log, arc20
      WHERE receipt._id = receipt_log.receipt_id AND receipt_log.address = arc20.contract_address AND ${logFilter}
    `, {type: db.QueryTypes.SELECT, transaction: this.ctx.state.transaction})
    ...
    let ids = (await db.query(sqlRaw`
      ...
      INNER JOIN (
        SELECT DISTINCT(receipt.transaction_id) AS id FROM receipt, receipt_log, arc20
        WHERE receipt._id = receipt_log.receipt_id AND receipt_log.address = arc20.contract_address AND ${logFilter}
    ...
    if (list.length > 0) {
      let intialBalanceList = await ARC20Balance.findAll({
        where: {
      ...
      for (let log of logs) {
        ...
        if (item) {
          ...
          result.tokens.push({
            ...
            name: log.arc20.name.toString(),
            symbol: log.arc20.symbol.toString(),
            decimals: log.arc20.decimals,
            amount: delta
          })
        }
      }
  ...
  async getARC20TokenRichList(contractAddress) {
    const db = this.ctx.model
    const {Qrc20Balance: ARC20Balance} = db
    const {ne: $ne} = this.app.Sequelize.Op
    let {limit, offset} = this.ctx.state.pagination

    let totalCount = await ARC20Balance.count({
      where: {contractAddress, balance: {[$ne]: Buffer.alloc(32)}},
      transaction: this.ctx.state.transaction
    })
    let list = await ARC20Balance.findAll({
      where: {contractAddress, balance: {[$ne]: Buffer.alloc(32)}},
